...
export class MenuService implements OnDestroy {
  private _showSidebar$ = new BehaviorSubject<boolean>(true);  // creates a private BehaviorSubject with the initial value of true for showing the sidebar
  private _showMobileMenu$ = new BehaviorSubject<boolean>(false);  // creates a private BehaviorSubject with the initial value of false for showing the mobile menu
  public _pagesMenu$ = new BehaviorSubject<MenuItem[]>([]);  // creates a public BehaviorSubject for the menu items with an empty array as initial value
  private subscription = new Subscription();  // creates a private Subscription variable to manage subscription

  constructor(private router: Router) {  // injects the Router service
    /** Set dynamic menu */
    this._pagesMenu$.next(Menu.pages);  // set the initial value of _pagesMenu$ to the Menu.pages array

    let sub = this.router.events.subscribe((event) => {  // creates a subscription to router events
      if (event instanceof NavigationEnd) {
        /** Expand menu base on active route */
        this._pagesMenu$.forEach((menuItem) => {  // loops through the menu items
          menuItem.forEach((menu) => {  // loops through the sub-menu items
            let activeGroup = false;
            menu.items.forEach((subMenu) => {  // loops through the sub-items
              const active = this.isActive(subMenu.route);  // checks if the sub-item's route is active
  ...
  get showSideBar$() {  // getter function for the _showSidebar$ BehaviorSubject
    return this._showSidebar$.asObservable();
  }
  get showMobileMenu$() {  // getter function for the _showMobileMenu$ BehaviorSubject
    return this._showMobileMenu$.asObservable();
  }
  get pagesMenu$() {  // getter function for the _pagesMenu$ BehaviorSubject
    return this._pagesMenu$.asObservable();
  }

  set showSideBar(value: boolean) {  // setter function for the _showSidebar$ BehaviorSubject
    this._showSidebar$.next(value);
  }
  set showMobileMenu(value: boolean) {  // setter function for the _showMobileMenu$ BehaviorSubject
    this._showMobileMenu$.next(value);
  }

  public toggleSidebar() {  // function to toggle the sidebar
  ...
  public toggleMenu(menu: any) {  // function to toggle the menu
